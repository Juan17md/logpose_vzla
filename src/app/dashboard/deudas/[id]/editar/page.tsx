...
export default function EditarDeudaPage() {
    ...
    const { debts, loadingDebts, updateDebt } = useDebts();
    ...
    const handleSubmit = async (data: any) => {
        setIsLoading(true);
        const success = await updateDebt(id, data);
        setIsLoading(false);
        if (success) {
        ...
        <div className="space-y-6 max-w-4xl mx-auto pb-10">
            ...
            <div className="bg-linear-to-br from-slate-900/80 to-slate-900/40 border border-slate-700/50 p-6 md:p-8 rounded-3xl shadow-xl relative overflow-hidden backdrop-blur-xl">
                ...
                <div className="relative z-10">
                    ...
                    <h1 className="text-2xl md:text-3xl font-bold text-white mb-2 tracking-tight">Editar Deuda</h1>
                    <p className="text-slate-400 text-sm">Actualiza los detalles del compromiso financiero.</p>
                </div>
            </div>
            ...
            >
                <DebtForm initialData={debt} onSubmit={handleSubmit} isLoading={isLoading} />
            </motion.div>
        </div>
    );
}
